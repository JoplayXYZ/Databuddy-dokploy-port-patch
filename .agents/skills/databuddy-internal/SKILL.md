...
- Slack routing is organization-scoped: OAuth binds a Slack workspace to a Databuddy organization, `/bind` is the only slash command and is now mainly an explicit approval step for Slack Connect/unknown channels; normal internal channels auto-bind on first mention. DMs/assistant threads work after workspace install. Analytics questions should go through app mentions/DMs using MCP-style website discovery instead of website/domain bindings. Slack emits evlog events under `apps/slack/.evlog/logs` in development/`SLACK_EVLOG_FS=1`; Axiom uses `AXIOM_TOKEN` with `SLACK_AXIOM_DATASET` defaulting to `slack`; and reactions need the `reactions:write` bot scope. Remote manifest updates need `SLACK_APP_ID` plus a Slack app configuration token in `SLACK_APP_CONFIG_TOKEN`; trust Slack API errors over token-prefix guesses.
