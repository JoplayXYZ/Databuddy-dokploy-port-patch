...
export async function classifySlackThreadReplyRelevance({
	...
	try {
		const result = await generateObject({
			...
			system:
				"Classify whether Databuddy should answer the latest Slack Message. Thread context is context only; decide on Message. shouldReply=true if Message mentions the bot, asks/continues a request, asks analytics/data/product help, answers Databuddy's previous clarification/follow-up (both, first one, landing page, yes), or is clearly social/banter directed at Databuddy where a short likable reply helps (compliment requests, good bot, playful insults, i hate you, shut up). shouldReply=false for ambient jokes, thanks-only, chatter, profanity/reactions not clearly addressed to Databuddy, status comments, or messages to another human. reason=bot_mentioned only if Message contains the bot mention token.",
			prompt: [
