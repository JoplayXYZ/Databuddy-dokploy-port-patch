...
test("updates the signed-in user's profile name", async ({
	authenticatedPage,
	e2eSession,
}) => {
	...
	await expect(authenticatedPage.getByText("You have unsaved changes")).toBeVisible();
	await authenticatedPage.getByRole("button", { name: "Save Changes" }).click();

	await expect(authenticatedPage.getByText("You have unsaved changes")).toBeHidden();
