...
test("creates and deletes an API key without leaving confirmation dialogs open", async ({
	authenticatedPage,
	e2eSession,
}) => {
	...
	await expect(authenticatedPage.getByText("Secret key", { exact: true })).toBeVisible();
