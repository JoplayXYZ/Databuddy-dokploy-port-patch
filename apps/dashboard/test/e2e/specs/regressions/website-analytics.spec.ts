...
test(
	...
	async ({ authenticatedPage, e2eSession }) => {
		...
		await expect(
			authenticatedPage.getByRole("radio", { name: "Daily" })
		).toBeVisible();
		await expect(
			authenticatedPage.getByRole("radio", { name: "Hourly" })
		).toBeVisible();
		await expect(
			authenticatedPage.getByRole("button", { name: "24h" })
		).toBeVisible();
		await expect(
			authenticatedPage.getByRole("button", { name: "7d" })
		).toBeVisible();
		await expect(
			authenticatedPage.getByRole("button", { name: "30d" })
		).toBeVisible();
		await expect(
			authenticatedPage.getByRole("button", { name: "Filter" })
		).toBeVisible();
